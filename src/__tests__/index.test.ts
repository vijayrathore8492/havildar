...
describe("check", () => {
  ...
  test("404 endpoint", async () => {
    ...
    expect(response.body).toEqual({ code: 404, error: 'Not found.' });
  });

  test("default client error", async () => {
    const response = await request(router).get("/clientError");
    expect(response.status).toEqual(400);
    expect(response.body).toEqual({ code: 400, error: 'bad request' });
  });
  ...
  test("custom412Error client error", async () => {
    ...
    expect(response.body).toEqual({ code: 412, error: 'custom412Error occurred' });
  });
  ...
  describe("when on prod", () => {
    ...
    test("server error", async () => {
      ...
      expect(response.body).toEqual({ code: 500, error: 'Internal Server Error' });
    });
  })
});